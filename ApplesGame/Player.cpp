...
namespace ApplesGame
{
	...
	bool HasPlayerCollisionWithScreenBorder(const Player& player)
	{
		...
		//хотел бы сделать, чтобы пэкмен как в оригинале выходил с противоположной части экрана, но похоже это пока слишком сложно для меня
		// 
		//В итоге, сделал, чтобы пэкмен не проигрывал при столкновении (лишь появлялся снова в центре), кушал знакомые ему "квадртики"

		//К сожалению, простого способа, чтобы "квадратики" просто исчезали - не нашёл
		// (непонятно для меня, как именно происходит работа с конкретным "яблоком-квадратиком")
